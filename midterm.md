...
- Created an Azure Function App named restful-function using the Node.js 22 LTS runtime stack. We selected Canada Central as the deployment region due to regional limitations, which are explained in more detail later on
...
- Faced a policy restriction when trying to deploy in the US East region, so we switched to West US 2 for compatibility with the Azure for Students subscription.

## Creating a VM to test our Authentication

![alt text](image-20.png)

- Set up a a low-cost Azure Linux (Ubuntu) VM to test the authentication process independently using the B2s size to keep costs down while still meeting project needs. We originally tried creating a VM with the B1 size but faced issues when trying to use Remote-SSH to access the VM (it kept losing connection and reconnecting even after many attempts) so we opted for the second cheapest option after B1s.

- The VM is hosted in the West US 2 region using the Azure for Students subscription (we chose West US over East US due to the region policy error as explained before).
...
- All documentation and source code for our authentication server (auth-server) solution are available in the repository linked below, specifically within the README.md file. The auth-server was intentionally developed and maintained in a separate repository to align with software engineering best practices. This separation promotes cleaner project architecture, simplifies version control, and enhances modularity.
